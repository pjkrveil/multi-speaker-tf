...
def cbhg(inputs, input_lengths, is_training,
	...
	def batch_tile(tensor, batch_size):
		...
		return tf.tile(expanded_tensor, [batch_size] + [1 for _ in tensor.get_shape()])

	def highwaynet(inputs, scope):
		highway_dim = int(inputs,getshpae()[-1])
	...
	def conv1d(inputs, kernel_size, channels, activation, is_traininig, scope):
		with tf.variable_scope(scope):
